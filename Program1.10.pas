...
const
  ...
  defaultPeriod = 120;
type 
  ControlGame = (run, pause, exit, gameOver);
...
begin
  if keypressed then 
  begin
    if gameState = pause then gameState := run; 
    key := readkey;
    case key of
...
procedure PutFood;
begin
  food.X := random(screenWidth - 10) + 5;
  food.Y := random(screenHeigh -4) + 2;
...
begin
  Randomize;
  gameState := ControlGame.pause;
  period := defaultPeriod; //микросекунд
  snake.lenght := 1;
  snake.body[1].X := random(screenWidth - 10) +5;
  snake.body[1].y := random(screenHeigh -4) +2;
  PutFood;
end;

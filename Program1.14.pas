...
const
  ...
  gameArenaBgColor = Green;
  gameArenaFgColor = Black;
  uiBgColor = Yellow;
  uiFgColor = Black;
...
var 
  ...
  fillLine : String;
begin
  FillChar(fillLine, screenWidth, ' ');
  SetLength(fillLine, screenWidth);

  TextBackground(gameArenaBgColor);
  for counterLine := 1 to screenHeigh do
  begin
    GotoXY(1, counterLine);
    Write(fillLine);
  end;
end;
...
begin
  TextBackground(uiBgColor);
  TextColor(uiFgColor);
  gotoxy(1,1);// помещаем курсор в начало первой строки 
  write( 'Очков:', snake.lenght:5);//выводим данные
  
  if gamestate = ControlGame.gameOver then //если игра окончена
  begin
    GotoXY((screenWidth - Length(gameOverText)) div 2, screenHeigh div 2);// размещаем курсор по середине экрана
